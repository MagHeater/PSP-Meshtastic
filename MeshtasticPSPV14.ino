...
// ║                                                                          ║
// ║  [TABS] Doppelt so groß (höher, größere Schrift)                         ║
// ║                                                                          ║
// ║  [KAMERA] Nur 3 Auflösungen:                                             ║
// ║  - 320x240 Hoch (Q12)                                                    ║
// ║  - 320x240 Low  (Q25)                                                    ║
// ║  - 160x120      (Q30)                                                    ║
// ║  Kein Grayscale-Modus mehr, keine Prozent-Skalierungsbuttons             ║
// ║                                                                          ║
// ║  [SETTINGS] Neuer Sub-Tab "WIFI":                                        ║
// ║  - SSID umbenennen                                                       ║
// ║  - Passwort setzen (leer = offen)                                        ║
// ║  - Verstecktes WLAN (hidden SSID) togglebar                              ║
// ║  - Änderungen werden in NVS gespeichert, ESP startet WLAN neu            ║
// ║                                                                          ║
// ║  [PRESETS] Checkboxen pro Button: CH1 und/oder CH2 aktivierbar           ║
// ║  - Preset kann auf CH1, CH2 oder beides senden                           ║
...
// SSID/PW/Hidden aus NVS geladen (Defaults)
...
#define PHOTO_SEND_DELAY_MS  5000   // 5s zwischen Chunks beim Senden

// ═══════════════════════════════════════════════════════════════════════════
// IMGREQ — Retry fehlender Chunks
// ═══════════════════════════════════════════════════════════════════════════
#define IMGREQ_DELAY_MS   3000   // 3s zwischen Retry-Chunks
#define IMGREQ_MAX_ROUNDS    2   // max. 2 Retry-Runden pro Session

// ═══════════════════════════════════════════════════════════════════════════
// PRESET BUTTONS  (9 Stück)
// ch1En/ch2En: für welche Kanäle der Preset senden soll
// ═══════════════════════════════════════════════════════════════════════════
#define PRESET_COUNT 9
char presetLabels[PRESET_COUNT][24];
char presetTexts[PRESET_COUNT][64];
bool presetCh1[PRESET_COUNT];   // true = auf CH1 senden
bool presetCh2[PRESET_COUNT];   // true = auf CH2 senden
...
// ═══════════════════════════════════════════════════════════════════════════
// IMGREQ — Retry-Job (fehlende Chunks nachsenden)
// ═══════════════════════════════════════════════════════════════════════════
...
struct RetryJob {
  ...
  uint32_t      targetNode;
  uint16_t      sessionId;
  uint8_t*      jpegData;    // Zeiger auf Originaldaten (wird NICHT freigegeben)
  size_t        jpegLen;
  int           totalChunks;
  uint16_t      imgW;
  uint16_t      imgH;
  ...
  int           missingCount;
  int           nextIdx;     // Index in missingList[]
  unsigned long lastSentMs;
  int           roundsDone;
};
...
struct PhotoRecvSession {
  ...
  int       receivedCount;
  bool      complete;
  bool      reqSent;         // IMGREQ bereits gesendet?
  int       reqRounds;       // wie viele Retry-Runden wurden gesendet
...
// ═══════════════════════════════════════════════════════════════════════════
// SECURITY — MAC-Whitelist + Verbindungs-History
// ═══════════════════════════════════════════════════════════════════════════
#define SEC_MAX_WHITELIST   8
#define SEC_MAX_HISTORY    20
...
struct SecEvent {
  ...
  uint8_t  event;         // 0=connect 1=blocked 2=disconnect 3=learned
};
...
SecEvent  secHistory[SEC_MAX_HISTORY];
int       secHistoryHead     = 0;
int       secHistoryCount    = 0;
...
// ═══════════════════════════════════════════════════════════════════════════
// KAMERA STATE
// Qualität: 0=Hoch(320x240 Q12)  1=Low(320x240 Q25)  2=160x120(Q30)
// ═══════════════════════════════════════════════════════════════════════════
...
void handleSecMode();     void handleSecWlRemove(); void handleSecHistClear();
void handleWifiSave();
...
void secAddHistory(const char* mac, const char* ip, uint8_t event);
...
void loadWifiSettings() {
  ...
  String p = prefs.getString("wifi_pw", "");
  strncpy(wifiPassword, p.c_str(), 64); wifiPassword[64] = '\0';
...
void applyWifiSettings() {
  ...
  const char* pw = (strlen(wifiPassword) > 0) ? wifiPassword : nullptr;
  WiFi.softAP(wifiSSID, pw, 1, wifiHidden ? 1 : 0, 1);
...
void secAddHistory(const char* mac, const char* ip, uint8_t event) {
  SecEvent& e = secHistory[secHistoryHead % SEC_MAX_HISTORY];
  strncpy(e.mac, mac, 17); e.mac[17] = '\0';
  strncpy(e.ip,  ip,  15); e.ip[15]  = '\0';
  e.timestamp_s = (uint32_t)(millis() / 1000);
  e.event = event;
  secHistoryHead = (secHistoryHead + 1) % SEC_MAX_HISTORY;
  if (secHistoryCount < SEC_MAX_HISTORY) secHistoryCount++;
}
...
bool secCheckMAC(const char* mac) {
  if (!secWhitelistMode) {
    if (!secMACinWhitelist(mac) && secWhitelistCount < SEC_MAX_WHITELIST) {
      ...
      secWhitelistCount++;
      secSave();
      secAddHistory(mac, secCurrentIp, 3);
...
void secOnConnect(const String& mac, const String& ip) {
  ...
  if (allowed) {
    secAddHistory(mac.c_str(), ip.c_str(), 0);
    secClientKnown = true;
    Serial.printf("[SEC] Client OK: %s / %s\n", mac.c_str(), ip.c_str());
  } else {
    secAddHistory(mac.c_str(), ip.c_str(), 1);
...
void secOnDisconnect(const String& mac) {
  secAddHistory(mac.c_str(), secCurrentIp, 2);
  secClientKnown = false;
...
void setup() {
  ...
  delay(1500);
  Serial.println("--- Meshtastic PSP v14 (2026-06-12T14:00:00Z) ---");
  ...
  server.on("/sec_hist_clr", [](){ handleSecHistClear(); });
  server.begin();
...
// ═══════════════════════════════════════════════════════════════════════════
// KAMERA INIT
// 0=320x240 Q12  1=320x240 Q25  2=160x120 Q30  (alle Farbe)
// ═══════════════════════════════════════════════════════════════════════════
bool initCamera(uint8_t quality) {
  esp_camera_deinit();
  delay(150);
  ...
  if (s) {
    ...
    s->set_special_effect(s,0); // immer Farbe
  }
  delay(300);
...
// ═══════════════════════════════════════════════════════════════════════════
// PRESET BUTTONS — NVS
// Format: prs<i>=text  prc<i>=flags  (bit0=CH1 bit1=CH2)
// ═══════════════════════════════════════════════════════════════════════════
void savePresets() {
...
void loadPresets() {
  ...
  for(int i=0;i<PRESET_COUNT;i++){
    ...
    char key2[12]; snprintf(key2,sizeof(key2),"prc%d",i);
    uint8_t flags=prefs.getUChar(key2, 2); // Default: CH2 an
    presetCh1[i] = (flags & 1) != 0;
    presetCh2[i] = (flags & 2) != 0;
    if(!presetCh1[i]&&!presetCh2[i]) presetCh2[i]=true; // mindestens einer
  }
}
...
String frameCss(){
  return "<style>"
    ...
    ".sec-learned{color:#00ccff;font-size:9px}"
    ".sec-disc{color:#555555;font-size:9px}"
...
void handleSecHistClear() {
  memset(secHistory,0,sizeof(secHistory)); secHistoryCount=0; secHistoryHead=0;
  server.sendHeader("Location", "/settings?subtab=sec"); server.send(303);
}
...
void handleShell(){
  ...
  html+="<html><head><meta charset='UTF-8'><title>Mesh PSP v14</title></head>";
  // Tabs nehmen 100px (ca. doppelt so hoch wie v13)
  html+="<frameset rows='100,*' border='0' frameborder='0' framespacing='0'>";
...
void handleTabs(){
  ...
  h+="<style>"
    "body{background:#111;margin:0;padding:3px 2px;font-family:monospace;"
      "white-space:nowrap;overflow:hidden;display:flex;gap:2px;height:94px;box-sizing:border-box}"
    // Tabs sind jetzt deutlich höher und größere Schrift
  ...
  if(anyPreset){
    ...
    for(int i=0;i<PRESET_COUNT;i++){
      ...
      html+="<a class='preset-btn' href='/send?target="+
            ...
            "&tab="+String(tab)+"&msg="+String(presetTexts[i])+"&refresh=1' target='_self'>"+
            escHtml(lbl)+"</a>";
    }
    html+="</div>";
  }
...
// Sub-Tab via ?subtab=presets|sec|wifi
// ═══════════════════════════════════════════════════════════════════════════
void handleSettings(){
  ...
  if(subtab=="presets"){
    ...
    html+="<div class='set-label'>QUICK-SEND BUTTONS &mdash; 9 Slots &mdash; Haken: CH1 / CH2</div>";
  ...
  // ── WIFI ─────────────────────────────────────────────────────────────────
    ...
    html+="<div style='color:#334455;font-size:9px;margin-bottom:6px'>"
          ...
          +" &nbsp;|&nbsp; Passwort: <span style='color:#44aaff'>"
          +String(strlen(wifiPassword)>0?"gesetzt ("+String(strlen(wifiPassword))+" Zeichen)":"offen")
          +"</span>"
          +" &nbsp;|&nbsp; Versteckt: <span style='color:#44aaff'>"
          +String(wifiHidden?"JA":"NEIN")+"</span></div>";
    ...
    html+="<div class='set-row'>";
    html+="<span class='set-label' style='min-width:70px'>Passwort&nbsp;</span>";
    html+="<input class='set-inp-wifi' type='text' name='wifi_pw' "
          "value='"+escHtml(wifiPassword)+"' maxlength='63' "
          "placeholder='leer = offen (kein Passwort)' style='flex:1'>";
    html+="</div>";
    ...
    html+="<label for='wifi_hidden' style='color:#778899;font-size:10px'>"
          "Verstecktes WLAN (SSID nicht sichtbar, Verbindung nur per manuellem Eintrag)</label>";
    html+="</div>";
    ...
    html+="<div style='color:#222;font-size:9px;margin-top:6px'>"
          "ESP startet WLAN sofort neu. Dein Gerät verliert kurz die Verbindung.</div>";
    html+="</form></div>";
  }
  ...
  // ── SECURITY ─────────────────────────────────────────────────────────────
    ...
    html+="<div class='set-label'>STATUS</div>";
    ...
    if(secWhitelistCount==0){
      html+="<div style='color:#333;font-size:9px'>Leer. Verbinde dein Gerät im Lernmodus.</div>";
    } else {
      for(int i=0;i<secWhitelistCount;i++){
    ...
    html+="<hr><div class='set-label'>VERBINDUNGS-HISTORY "
          "&nbsp;<a class='sec-btn' href='/sec_hist_clr'>[Löschen]</a></div>";
    if(secHistoryCount==0){
      html+="<div style='color:#333;font-size:9px'>Keine Einträge.</div>";
    } else {
      for(int k=0;k<secHistoryCount;k++){
        int idx=((secHistoryHead-1-k)+SEC_MAX_HISTORY)%SEC_MAX_HISTORY;
        SecEvent& e=secHistory[idx];
        const char* evtCls; const char* evtTxt;
        switch(e.event){
          case 0: evtCls="sec-ok";      evtTxt="VERBUNDEN"; break;
          case 1: evtCls="sec-blocked"; evtTxt="GEBLOCKT";  break;
          case 2: evtCls="sec-disc";    evtTxt="GETRENNT";  break;
          case 3: evtCls="sec-learned"; evtTxt="GELERNT";   break;
          default:evtCls="sec-disc";    evtTxt="?";         break;
        }
        html+="<div style='border-bottom:1px solid #111;padding:2px 0;font-size:9px'>";
        html+="<span style='color:#333'>+"+String(e.timestamp_s)+"s</span> ";
        html+="<span class='sec-mac'>"+escHtml(e.mac)+"</span> ";
        html+="<span style='color:#556'>"+escHtml(e.ip)+"</span> ";
        html+="<span class='"+String(evtCls)+"'>"+String(evtTxt)+"</span>";
        html+="</div>";
      }
    }
  ...
  html+="<hr><div style='color:#333;font-size:9px;margin-top:4px'>"
        "Mesh PSP v14 &mdash; 2026-06-12T14:00:00Z</div>";
  html+="</body></html>";
  server.send(200,"text/html; charset=utf-8",html);
}
...
void handleSettingsSave(){
  for(int i=0;i<PRESET_COUNT;i++){
    ...
    // Sicherheitsnetz: mindestens ein Kanal aktiv wenn Text gesetzt
...
// ═══════════════════════════════════════════════════════════════════════════
// KAMERA HAUPTSEITE
// Nur 3 Auflösungs-Buttons, keine %-Skalierungsbuttons
// ═══════════════════════════════════════════════════════════════════════════
void handleCamera(){
  ...
  // Auflösungs-Labels (kein Grayscale mehr)
  ...
  // Auflösungs-Buttons (flex, keine Prozent-Buttons)
  ...
  if(capturedJpeg!=nullptr&&capturedJpegLen>0){
    ...
    // Vorschau-Button (nur Vollbild, kein Größen-Select)
