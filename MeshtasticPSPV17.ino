...
// ║  [CAM-FIX] set_ae_level: 2 -> 0  (war Hauptursache dunkler Bilder)      ║
// ║  [CAM-FIX] set_aec_value: 900 -> 400 (höhere Belichtungszeit)           ║
// ║  [CAM-FIX] set_gainceiling: 6 -> 2 (weniger Rauschen)                   ║
// ║  [CAM-FIX] Warmup-Delay: 100ms -> 30ms (wie PSP WebCam)                 ║
// ║  [CAM-FIX] handleCamSnap: Sensor-Lock-Release vor Capture               ║
// ║  [CAM-FIX] Re-Init der Kamera vor jedem Snap für stabile Belichtung  ║
// ║                                                                          ║
...
void setup() {
  ...
  delay(1500);
  Serial.println("--- Meshtastic PSP v16 OV3660-FIX ---");
...
bool initCamera(uint8_t quality) {
  ...
  // OV3660: Power-Cycle für sauberen Start (wie PSP WebCam)
  ...
  if (s) {
    ...
    // ── Belichtung — FIX gegenüber v16 ───────────────────────────────────
    s->set_exposure_ctrl(s, 1);    // AEC an
    //s->set_aec2(s, 1);             // Erweiterte AEC an
    s->set_ae_level(s, 0);         // FIX: war 2 → OV3660 braucht 0, sonst zu dunkel
   // s->set_aec_value(s, 400);      // FIX: war 900 → 400 gibt längere Belichtungszeit

    // ── Gain — FIX gegenüber v16 ─────────────────────────────────────────
    s->set_gain_ctrl(s, 1);        // AGC an
   // s->set_agc_gain(s, 0);
    //s->set_gainceiling(s, (gainceiling_t)2); // FIX: war 6 → 2 reduziert Rauschen
  ...
  delay(300);
  // Warmup-Frames — FIX: 30ms statt 100ms (wie PSP WebCam)
  for (int i = 0; i < 6; i++) {
...
void handleShell(){
  ...
  html+="<html><head><meta charset='UTF-8'><title>Mesh PSP v16 OV3660</title></head>";
...
void handleCamSnap(){
  ...
  if(!cameraReady){
    ...
    // FIX: Kamera neu initialisieren vor jedem Snap (wie PSP WebCam camDeinit/camEnsureInit)
    // Gibt stabile und helle Belichtung, kostet ~500ms
    cameraReady = initCamera(camQuality);
    if(!cameraReady){
      snapStatus="<div style='color:#ff4444'>Kamera Re-Init fehlgeschlagen.</div>";
    } else {
      // FIX: Sensor AGC/AEC Lock aufheben vor Capture (wie PSP WebCam takePicture)
      sensor_t* s = esp_camera_sensor_get();
      if(s){
        s->set_gain_ctrl(s, 0);
      ...
      // Flush: alten gepufferten Frame verwerfen
      camera_fb_t* ex = esp_camera_fb_get();
      if(ex){ esp_camera_fb_return(ex); delay(80); }

      // Auf AEC einpendeln warten
      delay(150);

      // Eigentlicher Capture
      camera_fb_t* fb = esp_camera_fb_get();

      // AGC/AEC wieder auf Auto setzen (für nächsten Snap)
      if(s){
        s->set_gain_ctrl(s, 1);
