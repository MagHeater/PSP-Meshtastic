...
// ║  Meshtastic PSP  —  v18                                                  ║
// ║  Erstellt:  2026-06-23                                                   ║
// ║  Änderungen gegenüber v16 OV3660-FIX:                                   ║
// ║  [DISCO] sendDisconnect() vor Deep Sleep eingefügt                       ║
// ║  [DISCO] XIAO nRF bekommt sauberes Disconnect-Signal                     ║
// ║  [DISCO] 300ms Wartezeit damit XIAO Session zurücksetzt                  ║
// ║  [DISCO] Vorwärtsdeklaration ergänzt                                     ║
// ║                                                                          ║
...
void sendDisconnect();   // [v18] Sauberes Abmelden vom XIAO nRF
...
void setup() {
  ...
  delay(1500);
  Serial.println("--- Meshtastic PSP v18 ---");
  ...
  server.on("/shutdown", [](){
    ...
    // [v18] Sauberes Abmelden — XIAO setzt Serial-Session zurück
    sendDisconnect();
    delay(300);
...
// ═══════════════════════════════════════════════════════════════════════════
// [v18] DISCONNECT — Sauberes Abmelden vom XIAO nRF vor Deep Sleep
// ═══════════════════════════════════════════════════════════════════════════
void sendDisconnect(){
  meshtastic_ToRadio tr=meshtastic_ToRadio_init_zero;
  ...
  if(pb_encode(&s,meshtastic_ToRadio_fields,&tr)){
    ...
    Serial2.write((s.bytes_written>>8)&0xFF); Serial2.write(s.bytes_written&0xFF);
    Serial2.write(buf,s.bytes_written); Serial2.flush();
    Serial.println("[v18] Disconnect gesendet an XIAO nRF");
  }
}
...
void handleShell(){
  ...
  html+="<html><head><meta charset='UTF-8'><title>Mesh PSP v18</title></head>";
