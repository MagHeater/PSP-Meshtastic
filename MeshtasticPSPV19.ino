...
// ║  Erstellt:  2026-06-27                                                   ║
// ║                                                                          ║
// ║  Änderungen gegenüber v18:                                               ║
// ║  [ACK]   want_ack=true für PN & Channel-Nachrichten                      ║
// ║  [ACK]   Routing-Pakete (ROUTING_APP) werden ausgewertet                 ║
// ║  [ACK]   Gesendete Nachrichten erhalten [ok] / [ERR] Suffix im Chat      ║
// ║  [UI]    Settings: Ansichtsgrösse wählbar (PSP / Netbook / Smartphone)   ║
// ║  [UI]    Viewport + Font-Skalierung je nach gewähltem Gerät              ║
// ║  [NVS]   Fresh-Flash-Fix: vollständige Erstinitialisierung aller Keys    ║
// ║  [NVS]   Kein v12-Flash mehr nötig vor v19                               ║
// ║                                                                          ║
// ║  OV3660-Kamera-Fix: Belichtungseinstellungen von PSP WebCam v2.0         ║
// ║                                                                          ║
// ║                                                                          ║
...
// ANSICHTSGRÖSSE
// ═══════════════════════════════════════════════════════════════════════════
// 0 = PSP (default, wie bisher, kein Viewport-Tag)
// 1 = Netbook (viewport width=900)
// 2 = Smartphone (viewport width=device-width, grössere Fonts)
uint8_t uiScale = 0;
...
// [ACK] ACK-Status pro Message-Slot
// 0 = kein ACK nötig / unbekannt
// 1 = warte auf ACK (want_ack gesetzt, Packet-ID bekannt)
// 2 = ACK erhalten (ok)
// 3 = NACK / Fehler
struct ChatMessage {
  uint32_t senderId;
  ...
  uint8_t  ackStatus;   // [ACK] 0=n/a 1=pending 2=ok 3=err
};
...
// [ACK] Laufender Packet-ID-Zähler für eigene gesendete Pakete
...
void handleWifiSave();    void handleUiScaleSave();
...
// ═══════════════════════════════════════════════════════════════════════════
// [ACK] Haken-Badge  (kein Emoji — ASCII-only, PSP-kompatibel)
// ═══════════════════════════════════════════════════════════════════════════
String ackBadge(uint8_t ackStatus) {
  switch (ackStatus) {
...
// ═══════════════════════════════════════════════════════════════════════════
// [UI] Viewport & Font-Scale je nach uiScale
// ═══════════════════════════════════════════════════════════════════════════
String viewportTag() {
  switch (uiScale) {
    case 1: // Netbook
      return "<meta name='viewport' content='width=900'>";
    case 2: // Smartphone
      return "<meta name='viewport' content='width=device-width,initial-scale=1'>";
    default: // PSP — kein Viewport-Tag, Browser skaliert selbst
      return "";
  }
}
...
String fontScaleCss() {
  switch (uiScale) {
    case 2: // Smartphone: alle Fonts grösser
...
// ═══════════════════════════════════════════════════════════════════════════
// [NVS] Erstinitialisierung — Fresh-Flash-Fix
// Wird nur einmal ausgeführt wenn "nvs_v19" noch nicht gesetzt ist.
// Schreibt alle Keys mit Defaults damit v19 ohne vorheriges v12-Flash läuft.
// ═══════════════════════════════════════════════════════════════════════════
void nvsFirstInit() {
  if (prefs.getBool("nvs_v19", false)) return; // bereits initialisiert

  Serial.println("[NVS] Erstinitialisierung v19 ...");
  ...
  // Presets — Defaults setzen falls nicht vorhanden
  ...
  for (int i = 0; i < PRESET_COUNT; i++) {
    ...
    if (!prefs.isKey(key2)) prefs.putUChar(key2, 3); // CH1+CH2 aktiv
  }
...
// ═══════════════════════════════════════════════════════════════════════════
// UI SCALE — Laden/Speichern
// ═══════════════════════════════════════════════════════════════════════════
void loadUiScale() {
  uiScale = prefs.getUChar("ui_scale", 0);
  if (uiScale > 2) uiScale = 0;
}

void saveUiScale() {
  prefs.putUChar("ui_scale", uiScale);
}
...
// ═══════════════════════════════════════════════════════════════════════════
void setup() {
  // Deep-Sleep-Wakeup-Check: ESP32-CAM wacht nur per Power-On auf,
  // ESP_RST_DEEPSLEEP tritt hier nie auf — Check entfernt (v18-Fix bleibt)
  delay(2000);

  Serial.begin(115200);
  Serial2.begin(38400, SERIAL_8N1, 13, 12);
  delay(1500);
  Serial.println("--- Meshtastic PSP v19 ---");
  ...
  // [NVS] Fresh-Flash-Fix: Erstinitialisierung aller Keys
  nvsFirstInit();
  ...
  // ── Shutdown: Disconnect senden, dann Deep Sleep ─────────────────────────
  server.on("/shutdown", [](){
...
// ═══════════════════════════════════════════════════════════════════════════
// KAMERA INIT — OV3660 FIX (Belichtung wie PSP WebCam v2.0)
// ═══════════════════════════════════════════════════════════════════════════
bool initCamera(uint8_t quality) {
  esp_camera_deinit();
  delay(150);
...
void sendDisconnect(){
  ...
  if(pb_encode(&s,meshtastic_ToRadio_fields,&tr)){
    ...
    Serial2.write((s.bytes_written>>8)&0xFF); Serial2.write(s.bytes_written&0xFF);
    Serial2.write(buf,s.bytes_written); Serial2.flush();
    Serial.println("[v19] Disconnect gesendet an XIAO nRF");
  }
}
...
// ═══════════════════════════════════════════════════════════════════════════
// [ACK] ACK aus Routing-Paket auflösen
// Das nRF sendet nach erfolgreichem Relay/Delivery ein Routing-Paket zurück.
// portnum = ROUTING_APP, decoded.payload enthält meshtastic_Routing.
// error_reason == NONE (0) => ACK ok, sonst Fehler.
// Das Paket hat im Feld "decoded.request_id" die ursprüngliche Packet-ID.
// ═══════════════════════════════════════════════════════════════════════════
void processAck(uint32_t requestId, bool success) {
  // Suche die gesendete Nachricht mit dieser Packet-ID und update ackStatus
  int total = min(messageCount, MAX_MESSAGES);
  for (int i = 0; i < total; i++) {
    if (messages[i].packetId == requestId && messages[i].ackStatus == 1) {
      messages[i].ackStatus = success ? 2 : 3;
      Serial.printf("[ACK] PacketID %u -> %s\n", requestId, success ? "ok" : "ERR");
      return;
    }
  }
}
    ...
    if(pkt.which_payload_variant==meshtastic_MeshPacket_decoded_tag){
      ...
      if(data.portnum==meshtastic_PortNum_ROUTING_APP){
        ...
        if(pb_decode(&rs,meshtastic_Routing_fields,&routing)){
          ...
          // request_id steht in data.request_id (Feld 6 im Data proto)
          if(data.request_id != 0){
            processAck(data.request_id, success);
          }
        }
        return;
      }
      ...
      if(data.portnum==meshtastic_PortNum_TEXT_MESSAGE_APP){
        ...
        if(tl>0){
          ...
          messages[idx].ackStatus=0; // empfangene Nachrichten brauchen kein ACK
...
// ═══════════════════════════════════════════════════════════════════════════
// [ACK] SENDEN — want_ack=true, Packet-ID vergeben
// ═══════════════════════════════════════════════════════════════════════════
void sendToRadio(const char* text,uint32_t targetDst,uint8_t channelIdx){
  meshtastic_ToRadio    tr=meshtastic_ToRadio_init_zero;
  ...
  // [ACK] Für Text-Nachrichten (keine Foto-Chunks) ACK anfordern
  ...
  if (wantAck) {
    ...
    if (gNextPacketId == 0) gNextPacketId = 1; // 0 ist reserviert
    pkt.want_ack = true;
    pktId = pkt.id;
  } else {
    pkt.want_ack = false;
  ...
  if(!isPhotoMsg){
    ...
    messages[idx].ackStatus=wantAck ? 1 : 0; // 1 = pending
...
void handleShell(){
  ...
  html+=viewportTag();
  html+="<title>Mesh PSP v19</title></head>";
...
void handleChat(){
  ...
  for(int i=0;i<count;i++){
    ...
    // [ACK] Haken hinter eigene Nachrichten
...
void handleSettings(){
  ...
  stab("view","ANSICHT","set-subtab-view");
  html+="</div>";
  ...
  // ── ANSICHT TAB ───────────────────────────────────────────────────────────
    ...
    html+="<div class='set-section'>";
    html+="<div class='set-label'>ANSICHTSGRÖSSE / ZIELGERÄT</div>";
    html+="<div style='color:#334433;font-size:9px;margin-bottom:8px'>"
          "Passt Schriftgrösse und Viewport an dein Gerät an.<br>"
          "Aktuell: <span style='color:#88ff44'>"
          +String(uiScale==0?"PSP":uiScale==1?"Netbook":"Smartphone")+"</span></div>";

    struct { uint8_t val; const char* lbl; const char* desc; } scales[]={
      {0,"PSP","480x272, Standardansicht, kein Viewport-Tag"},
      {1,"Netbook","900px-Viewport, leicht groessere Fonts"},
      {2,"Smartphone","device-width, grosse Touch-Fonts"},
    };
    html+="<div class='scale-row'>";
    for(auto& sc : scales){
      bool on=(uiScale==sc.val);
      html+="<a class='scale-btn"+String(on?" on":"")+"' "
           "href='/uiscalesave' "
           "onclick=\"this.href='/uiscalesave?scale="+String(sc.val)+"';return true;\">"
           +String(sc.lbl)+"</a>";
    }
    html+="</div>";
    // POST-Form als Fallback (PSP hat kein JS-Problem aber sicherer)
    html+="<form method='POST' action='/uiscalesave' style='margin-top:4px'>";
    html+="<div style='display:flex;gap:3px'>";
    for(int v=0;v<=2;v++){
      const char* lbl[]={"PSP","Netbook","Smartphone"};
      bool on=(uiScale==v);
      html+="<button type='submit' name='scale' value='"+String(v)+"' "
           "style='flex:1;background:"+String(on?"#152000":"#111")+";"
           "color:"+String(on?"#88ff44":"#555")+";border:1px solid "
           +String(on?"#447700":"#2a2a2a")+";padding:8px 2px;"
           "font-family:monospace;font-size:10px;cursor:pointer'>"+lbl[v]+"</button>";
    }
    html+="</div>";
    html+="</form>";
    html+="<div style='color:#222;font-size:9px;margin-top:6px'>"
          "PSP: 480px nativ, kein Zoom.<br>"
          "Netbook: Viewport 900px, etwas groessere Fonts.<br>"
          "Smartphone: Viewport device-width, Touch-optimiert.</div>";
    html+="</div>";
  }
  // ── SECURITY TAB ─────────────────────────────────────────────────────────
  else {
    ...
    if(strlen(secCurrentMac)>0){
      ...
      if(secClientKnown) html+="<div class='sec-ok'>[ok] Bekannt &mdash; verbunden</div>";
    ...
    for(int k=0;k<SEC_HISTORY_SIZE;k++){
      ...
      if(secHistory[i].allowed)
        html+="<span class='sec-hist-ok'>[ok]</span> ";
      else
    ...
    if(!secWhitelistMode){
      ...
      html+="<a class='sec-mode-lock' href='/sec_mode?v=1'>Lernmodus beenden -> SPERREN</a>";
    } else {
      html+="<div style='color:#ff6600;font-size:9px;margin-bottom:3px'>"
            "[!] Sperrmodus: Nur Whitelist-MACs erlaubt.</div>";
      html+="<a class='sec-mode-learn' href='/sec_mode?v=0'>Sperrmodus aufheben -> LERNEN</a>";
    }
    ...
    if(secWhitelistCount==0){
      ...
      for(int i=0;i<secWhitelistCount;i++){
        ...
        html+="</span>";
        html+="<a class='sec-btn-red' href='/sec_wl_rm?i="+String(i)+"'>[x]</a>";
        html+="</div>";
      }
    }
  ...
  html+="<a href='/shutdown' style='display:block;background:#1a0000;color:#ff4444;"
        ...
        "[off] ESP32 herunterfahren</a>";
  html+="<div style='color:#2a1010;font-size:8px;margin-top:3px'>"
        "Disconnect -> Deep Sleep -- Neustart nur per Reset/Strom</div>";
  html+="</div>";
  ...
  if(pnCount==0){
    ...
    for(int i=0;i<pnCount;i++){
      ...
      if(strncmp(preview,"FOTO:",5)==0)
        ...
        html+="<span class='tile-preview' style='color:#ffaa00'>[Retry laeuft...]</span>";
      else{
...
void handlePNConv(){
  ...
  for(int i=0;i<count;i++){
    ...
    // [ACK] Haken hinter eigene PN-Nachrichten
    html+="<div class='txt'>"+escHtml(messages[idx].text);
    if(isMe) html+=ackBadge(messages[idx].ackStatus);
    html+="</div></div>";
  }
  ...
  if(capturedJpeg!=nullptr&&capturedJpegLen>0){
    ...
    if(!photoJob.active&&!retryJob.active){
      ...
      String jobInfo = photoJob.active
        ? "Uebertragung laeuft... "+String(photoJob.nextChunk)+"/"+String(photoJob.totalChunks)
          +" ~"+String((photoJob.totalChunks-photoJob.nextChunk)*5)+"s"
        : "Retry laeuft... "+String(retryJob.nextIdx)+"/"+String(retryJob.missingCount)+" Chunks";
...
// ═══════════════════════════════════════════════════════════════════════════
// KAMERA SNAP — FIX: Re-Init + Sensor-Lock-Release wie PSP WebCam
// ═══════════════════════════════════════════════════════════════════════════
void handleCamSnap(){
  if(server.hasArg("pnnode"))
    activePnPartner=strtoul(server.arg("pnnode").c_str(),NULL,10);
  ...
  if(!cameraReady){
    ...
    cameraReady = initCamera(camQuality);
    if(!cameraReady){
      snapStatus="<div style='color:#ff4444'>Kamera Re-Init fehlgeschlagen.</div>";
    } else {
      sensor_t* s = esp_camera_sensor_get();
      if(s){
        s->set_gain_ctrl(s, 0);
  ...
  html+=frameCss(); html+="</head><body>";
  html+=snapStatus;
  html+="<div style='color:#555;font-size:9px;margin-top:4px'>Zurueck zur Kamera...</div>";
  html+="</body></html>";
  server.send(200,"text/html; charset=utf-8",html);
}
