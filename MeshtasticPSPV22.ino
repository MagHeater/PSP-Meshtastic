...
// ║  Erstellt:  2026-06-27                                                   ║
// ║                                                                          ║
// ║  Änderungen gegenüber v21:                                               ║
// ║  [FIX]   processAck() prüfte bisher NUR die Packet-ID, nicht den         ║
// ║           Absender (pkt.from) des Routing-Pakets. Dadurch konnte ein     ║
// ║           Zwischen-Hop, der die Nachricht nur weitergeleitet hat (und    ║
// ║           selbst ein Routing-Paket mit error_reason=NONE erzeugt),       ║
// ║           fälschlich als FINALE Zustellbestätigung gewertet werden.      ║
// ║           Symptom: Web-UI zeigt [ok], Nachricht kommt beim Ziel nie an.  ║
// ║  [FIX]   Bei PN (Unicast) wird das ACK jetzt nur noch akzeptiert, wenn   ║
// ║           pkt.from tatsächlich der Ziel-Node ist. Bei Broadcast          ║
// ║           (CH1/CH2) bleibt das alte Verhalten (kein eindeutiges Ziel).   ║
// ║  [DEBUG] Neues Serial-Log für JEDES eingehende Routing-Paket:            ║
// ║           zeigt from-Node, request_id und error_reason — damit im       ║
// ║           Seriellen Monitor sofort sichtbar ist, ob ein ACK wirklich     ║
// ║           vom Zielnode kommt oder nur von einem Relay.                   ║
// ║                                                                          ║
// ║  Änderungen gegenüber v20 (v21):                                         ║
// ║  [HOP]   Konfigurierbares Hop Limit in Settings (1–7, Default 3)         ║
// ║  [HOP]   Gilt für ALLE Pakete: Text CH1/CH2, PN, Traceroute              ║
// ║  [FIX]   hop_limit + hop_start werden jetzt in sendToRadio() gesetzt     ║
// ║  [FIX]   hop_limit + hop_start werden in sendTracerouteRequest() gesetzt ║
// ║  [FIX]   ACK-Empfang wieder funktionsfähig (war durch hop_limit=0 tot)   ║
// ║  [FIX]   Traceroute-Antwort kommt jetzt auch beim ESP an                 ║
// ║                                                                          ║
...
// ═══════════════════════════════════════════════════════════════════════════
// [HOP] KONFIGURIERBARES HOP LIMIT
// ═══════════════════════════════════════════════════════════════════════════
uint8_t hopLimit = 3; // Default: Meshtastic-Standard
...
struct ChatMessage {
  ...
  char     channelName[20];
  uint8_t  channelIdx;
  uint32_t packetId;    // [ACK] Packet-ID für ACK-Matching
  uint8_t  ackStatus;   // 0=n/a 1=pending 2=ok 3=err
};
...
// TRACEROUTE
// ═══════════════════════════════════════════════════════════════════════════
#define TRACE_TIMEOUT_MS     30000
#define TRACE_MAX_HOPS       8

struct TracerouteJob {
  bool          active;
  uint32_t      targetNode;
  uint32_t      packetId;
  unsigned long sentMs;
};
TracerouteJob traceJob = {false, 0, 0, 0};

struct TracerouteResult {
  bool      valid;
  uint32_t  targetNode;
  int       routeCount;
  uint32_t  route[TRACE_MAX_HOPS];
  int       snrTowards[TRACE_MAX_HOPS];
  int       snrTowardsCount;
  int       routeBackCount;
  uint32_t  routeBack[TRACE_MAX_HOPS];
  int       snrBack[TRACE_MAX_HOPS];
  int       snrBackCount;
  unsigned long receivedMs;
};
TracerouteResult lastTraceResult = {false,0,0,{},{},0,0,{},{},0,0};
...
static uint32_t gNextPacketId = 1;
...
void handleWifiSave();    void handleUiScaleSave();  void handleHopSave();
void handleTraceStart();  void handleTraceResult();
...
void sendTracerouteRequest(uint32_t targetNode);
void processTracerouteResponse(uint32_t fromNode, uint32_t requestId,
...
void loadUiScale();    void saveUiScale();
void loadHopLimit();   void saveHopLimit();
void nvsFirstInit();
...
String ackBadge(uint8_t ackStatus);
String viewportTag();
String fontScaleCss();
...
String ackBadge(uint8_t ackStatus) {
  switch (ackStatus) {
    case 1: return "<span style='color:#888;font-size:9px'> [...]</span>";
    case 2: return "<span style='color:#00cc44;font-size:9px'> [ok]</span>";
    case 3: return "<span style='color:#ff4444;font-size:9px'> [ERR]</span>";
    default: return "";
  }
}
...
String viewportTag() {
  switch (uiScale) {
    case 1: return "<meta name='viewport' content='width=900'>";
...
String fontScaleCss() {
  switch (uiScale) {
    case 2:
      return "html,body{font-size:15px!important}"
             ".sndr,.sndr-pn,.sndr-me,.sndr-sys,.meta,"
             ".node-id,.node-snr,.tele-label,.cam-status,"
             ".set-label,.sec-label,.set-idx,.rbar a{"
             "font-size:12px!important}"
             ".txt{font-size:14px!important}"
             "input[type=text]{font-size:16px!important;padding:6px!important}"
             "input[type=submit]{font-size:14px!important;padding:6px 12px!important}"
             ".preset-btn,.preset-btn-empty{font-size:12px!important;padding:5px 2px!important}"
             ".cam-btn-snap{font-size:17px!important;padding:15px!important}"
             ".pn-tile{padding:12px!important}"
             ".tile-name{font-size:15px!important}"
             ".tile-preview{font-size:13px!important}"
             ".tab{font-size:13px!important}";
    case 1:
      return "html,body{font-size:13px!important}"
...
// ═══════════════════════════════════════════════════════════════════════════
// [HOP] Hop Limit laden/speichern
// ═══════════════════════════════════════════════════════════════════════════
void loadHopLimit() {
  hopLimit = prefs.getUChar("hop_limit", 3);
  if (hopLimit < 1) hopLimit = 1;
  if (hopLimit > 7) hopLimit = 7;
}

void saveHopLimit() {
  prefs.putUChar("hop_limit", hopLimit);
}

// ═══════════════════════════════════════════════════════════════════════════
// NVS Erstinitialisierung
// ═══════════════════════════════════════════════════════════════════════════
void nvsFirstInit() {
  if (prefs.getBool("nvs_v21", false)) return;

  Serial.println("[NVS] Erstinitialisierung v21/v22 ...");

  if (!prefs.isKey("wifi_ssid"))   prefs.putString("wifi_ssid", "XIAO_MONITOR");
  if (!prefs.isKey("wifi_hidden")) prefs.putBool("wifi_hidden", false);
  if (!prefs.isKey("hop_limit"))   prefs.putUChar("hop_limit", 3);

  const char* presetDefaults[PRESET_COUNT] = {"go","stp","wg2","arm","drm","","","",""};
  for (int i = 0; i < PRESET_COUNT; i++) {
    char key[12]; snprintf(key, sizeof(key), "prs%d", i);
    if (!prefs.isKey(key)) prefs.putString(key, presetDefaults[i]);
    char key2[12]; snprintf(key2, sizeof(key2), "prc%d", i);
    if (!prefs.isKey(key2)) prefs.putUChar(key2, 3);
  }

  if (!prefs.isKey("favs"))       prefs.putString("favs", "");
  if (!prefs.isKey("sec_mode"))   prefs.putBool("sec_mode", false);
  if (!prefs.isKey("sec_wl_cnt")) prefs.putInt("sec_wl_cnt", 0);

  prefs.putInt("sec_h_head", 0);
  for (int i = 0; i < SEC_HISTORY_SIZE; i++) {
    ...
    if (!prefs.isKey(km)) prefs.putString(km, "");
    if (!prefs.isKey(ki)) prefs.putString(ki, "");
    if (!prefs.isKey(ko)) prefs.putBool(ko, false);
    if (!prefs.isKey(kt)) prefs.putUInt(kt, 0);
    if (!prefs.isKey(kv)) prefs.putBool(kv, false);
  }

  if (!prefs.isKey("ui_scale")) prefs.putUChar("ui_scale", 0);

  prefs.putBool("nvs_v21", true);
  prefs.putBool("nvs_v19", true);
  prefs.putBool("nvs_init", true);
  Serial.println("[NVS] Erstinitialisierung abgeschlossen.");
}

// ═══════════════════════════════════════════════════════════════════════════
// UI Scale
// ═══════════════════════════════════════════════════════════════════════════
void loadUiScale() { uiScale = prefs.getUChar("ui_scale", 0); if (uiScale > 2) uiScale = 0; }
void saveUiScale() { prefs.putUChar("ui_scale", uiScale); }
...
void secOnConnect(const String& mac, const String& ip) {
  ...
  else { secClientKnown = false; secAlertBlink(SEC_ALERT_BLINKS); Serial.printf("[SEC] Client GEBLOCKT: %s / %s\n", mac.c_str(), ip.c_str()); }
}
...
void setup() {
  ...
  delay(1500);
  Serial.println("--- Meshtastic PSP v22 ---");
  ...
  server.on("/uiscalesave",   HTTP_POST, handleUiScaleSave);
  server.on("/hopsave",       HTTP_POST, handleHopSave);
  server.on("/sec_mode",      [](){ handleSecMode(); });
  server.on("/sec_wl_rm",     [](){ handleSecWlRemove(); });
  server.on("/tracestart",    handleTraceStart);
  server.on("/traceresult",   handleTraceResult);
...
void loop() {
  ...
  if (millis() - lastSecCheck > 8000) {
    lastSecCheck = millis();
    if (secWhitelistMode && strlen(secCurrentMac) > 0 && !secClientKnown) secAlertBlink(2);
  }
...
// ═══════════════════════════════════════════════════════════════════════════
void tickTraceJob() {
  if (!traceJob.active) return;
  if (millis() - traceJob.sentMs > TRACE_TIMEOUT_MS) {
    Serial.println("[TRACE] Timeout.");
    traceJob.active = false;
  }
}
...
bool initCamera(uint8_t quality) {
  ...
  if (psramFound()) {
    cfg.frame_size=fs; cfg.jpeg_quality=jpegQ; cfg.fb_count=2;
    cfg.fb_location=CAMERA_FB_IN_PSRAM;
  } else {
    cfg.frame_size=FRAMESIZE_QVGA; cfg.jpeg_quality=15; cfg.fb_count=1;
  ...
  if (s) {
    ...
    s->set_exposure_ctrl(s,1); s->set_ae_level(s,0); s->set_gain_ctrl(s,1);
    s->set_bpc(s,1); s->set_wpc(s,1); s->set_raw_gma(s,1); s->set_lenc(s,1);
    s->set_brightness(s,1); s->set_saturation(s,-2); s->set_contrast(s,0);
    s->set_sharpness(s,1); s->set_denoise(s,1); s->set_special_effect(s,0);
  }
...
void loadFavorites() { Serial.printf("[Favs] %s\n", prefs.getString("favs","").c_str()); }
...
void sendDisconnect(){
  ...
  if(pb_encode(&s,meshtastic_ToRadio_fields,&tr)){
    ...
    Serial2.write((s.bytes_written>>8)&0xFF); Serial2.write(s.bytes_written&0xFF);
    Serial2.write(buf,s.bytes_written); Serial2.flush();
    Serial.println("[v22] Disconnect gesendet.");
  }
}

// ═══════════════════════════════════════════════════════════════════════════
// [FIX+HOP] TRACEROUTE SENDEN
// ═══════════════════════════════════════════════════════════════════════════
void sendTracerouteRequest(uint32_t targetNode) {
  if (targetNode == 0 || targetNode == 0xFFFFFFFF) return;
  ...
  uint8_t rdBuf[32];
  pb_ostream_t rdStream = pb_ostream_from_buffer(rdBuf, sizeof(rdBuf));
  bool rdOk = pb_encode(&rdStream, meshtastic_RouteDiscovery_fields, &rd);

  data.portnum = meshtastic_PortNum_TRACEROUTE_APP;
  if (rdOk) {
    data.payload.size = rdStream.bytes_written;
    memcpy(data.payload.bytes, rdBuf, rdStream.bytes_written);
  } else {
    data.payload.size = 0;
  }
  ...
  if (gNextPacketId == 0) gNextPacketId = 1;
  pkt.want_ack  = true;
  ...
  if (!pb_encode(&s, meshtastic_ToRadio_fields, &tr)) {
    Serial.println("[TRACE] Encode fehlgeschlagen.");
    return;
  }
  ...
  traceJob.active     = true;
  traceJob.targetNode = targetNode;
  traceJob.packetId   = pkt.id;
  traceJob.sentMs     = millis();
  lastTraceResult.valid = false;

  Serial.printf("[TRACE] Anfrage an 0x%08X (PacketID %u, HopLimit %d)\n",
                targetNode, pkt.id, hopLimit);
}

// ═══════════════════════════════════════════════════════════════════════════
// Traceroute Antwort verarbeiten
// ═══════════════════════════════════════════════════════════════════════════
void processTracerouteResponse(uint32_t fromNode, uint32_t requestId,
                                const uint8_t* payload, size_t payloadLen) {
  if (!traceJob.active) {
    Serial.println("[TRACE] Antwort ohne aktiven Job - ignoriert.");
    return;
  }
  bool matches = (requestId != 0 && requestId == traceJob.packetId) ||
                 (fromNode == traceJob.targetNode);
  if (!matches) {
    Serial.println("[TRACE] Antwort passt nicht - ignoriert.");
    return;
  }

  meshtastic_RouteDiscovery rd = meshtastic_RouteDiscovery_init_zero;
  pb_istream_t rs = pb_istream_from_buffer(payload, payloadLen);
  if (!pb_decode(&rs, meshtastic_RouteDiscovery_fields, &rd)) {
    Serial.println("[TRACE] RouteDiscovery decode fehlgeschlagen.");
    traceJob.active = false;
    return;
  }

  lastTraceResult.valid      = true;
  lastTraceResult.targetNode = traceJob.targetNode;
  lastTraceResult.receivedMs = millis();

  lastTraceResult.routeCount = min((int)rd.route_count, TRACE_MAX_HOPS);
  for (int i = 0; i < lastTraceResult.routeCount; i++)
    lastTraceResult.route[i] = rd.route[i];

  lastTraceResult.snrTowardsCount = min((int)rd.snr_towards_count, TRACE_MAX_HOPS);
  for (int i = 0; i < lastTraceResult.snrTowardsCount; i++)
    lastTraceResult.snrTowards[i] = rd.snr_towards[i] / 4;

  lastTraceResult.routeBackCount = min((int)rd.route_back_count, TRACE_MAX_HOPS);
  for (int i = 0; i < lastTraceResult.routeBackCount; i++)
    lastTraceResult.routeBack[i] = rd.route_back[i];

  lastTraceResult.snrBackCount = min((int)rd.snr_back_count, TRACE_MAX_HOPS);
  for (int i = 0; i < lastTraceResult.snrBackCount; i++)
    lastTraceResult.snrBack[i] = rd.snr_back[i] / 4;

  traceJob.active = false;
  Serial.printf("[TRACE] Antwort: %d Hops hin, %d zurueck\n",
                lastTraceResult.routeCount, lastTraceResult.routeBackCount);
}
...
void sendImgReqIfNeeded(PhotoRecvSession* s) {
  ...
  messages[idx].channelIdx=2; messages[idx].ackStatus=0;
...
void processPhotoDone(uint32_t senderNode,const char* msg){
  ...
  messages[idx].ackStatus=0; messageCount++;
}
...
// ═══════════════════════════════════════════════════════════════════════════
// [FIX-v22] ACK verarbeiten
// Prüft jetzt zusätzlich, ob das Routing-Paket wirklich vom ZIEL-Node kommt
// (pkt.from == targetId), nicht nur ob die request_id passt. Vorher konnte
// ein Zwischen-Hop, der die Nachricht bloß weitergeleitet hat, fälschlich
// als finale Zustellbestätigung gewertet werden ("ok" im UI, aber Ziel
// erhält die Nachricht nie). Bei Broadcast (targetId==0xFFFFFFFF) gibt es
// kein eindeutiges Ziel - dort bleibt das bisherige Verhalten erhalten.
// ═══════════════════════════════════════════════════════════════════════════
void processAck(uint32_t requestId, uint32_t fromNode, bool success) {
  int total = min(messageCount, MAX_MESSAGES);
  for (int i = 0; i < total; i++) {
    if (messages[i].packetId == requestId && messages[i].ackStatus == 1) {
      bool isUnicast = (messages[i].targetId != 0xFFFFFFFF);
      if (isUnicast && fromNode != messages[i].targetId) {
        Serial.printf("[ACK] PacketID %u: Routing-Paket von 0x%08X ignoriert "
                      "(Ziel war 0x%08X - nur Relay-Bestaetigung, kein finales ACK)\n",
                      requestId, fromNode, messages[i].targetId);
        return;
      }
      messages[i].ackStatus = success ? 2 : 3;
      Serial.printf("[ACK] PacketID %u von 0x%08X -> %s\n",
                    requestId, fromNode, success ? "ok" : "ERR");
      return;
    }
  }
}
  ...
  if(fr.which_payload_variant==meshtastic_FromRadio_packet_tag){
    ...
    if(pkt.which_payload_variant==meshtastic_MeshPacket_decoded_tag){
      ...
      if(data.portnum==meshtastic_PortNum_ROUTING_APP){
        ...
        if(pb_decode(&rs,meshtastic_Routing_fields,&routing)){
          bool success=(routing.error_reason==meshtastic_Routing_Error_NONE);
          // [DEBUG-v22] Immer loggen, unabhaengig davon ob ein passender
          // pending-Eintrag existiert - so seht ihr im Seriellen Monitor
          // sofort, WER das Routing-Paket geschickt hat.
          Serial.printf("[ROUTING] von 0x%08X, request_id=%u, error_reason=%d (%s)\n",
                        pkt.from, data.request_id, (int)routing.error_reason,
                        success ? "NONE/ok" : "FEHLER");
          if(data.request_id != 0) processAck(data.request_id, pkt.from, success);
        }
        return;
      }

      if(data.portnum==meshtastic_PortNum_TRACEROUTE_APP){
        processTracerouteResponse(pkt.from, data.request_id, data.payload.bytes, data.payload.size);
        return;
      }
...
// ═══════════════════════════════════════════════════════════════════════════
// [FIX+HOP] SENDEN — hop_limit und hop_start werden jetzt gesetzt
// ═══════════════════════════════════════════════════════════════════════════
void sendToRadio(const char* text,uint32_t targetDst,uint8_t channelIdx){
  meshtastic_ToRadio    tr=meshtastic_ToRadio_init_zero;
  meshtastic_MeshPacket pkt=meshtastic_MeshPacket_init_zero;
  ...
  bool isPhotoMsg = (strncmp(text,"IMG:",4)==0 ||
                     strncmp(text,"IMGDONE:",8)==0 ||
                     strncmp(text,"IMGREQ:",7)==0);
  bool wantAck = !isPhotoMsg;
  uint32_t pktId = 0;
  if (wantAck) {
    pkt.id = gNextPacketId++;
    if (gNextPacketId == 0) gNextPacketId = 1;
  ...
  if (wantAck) {
    Serial.printf("[SEND] PacketID %u an 0x%08X (HopLimit %d, want_ack=1)\n",
                  pktId, targetDst, hopLimit);
  }
  ...
  if(!isPhotoMsg){
    ...
    messages[idx].packetId=pktId;
    messages[idx].ackStatus=wantAck ? 1 : 0;
...
String frameCss(){
  ...
  return String("<style>") +
    ...
    ".btn-trace{background:#001018;color:#66ccff;border:1px solid #336688;"
      "padding:4px 8px;font-family:monospace;font-size:11px;text-decoration:none}"
    ...
    ".scale-row{display:flex;gap:3px;margin-bottom:6px}"
    ".scale-btn{flex:1;padding:7px 4px;background:#111;color:#555;border:1px solid #2a2a2a;"
      "font-family:monospace;font-size:10px;text-align:center;text-decoration:none}"
    ".scale-btn.on{background:#150d00;color:#ffaa00;border-color:#886600}"
    ".hop-row{display:flex;gap:3px;flex-wrap:wrap;margin-bottom:6px}"
    ".hop-btn{flex:1;min-width:28px;padding:7px 4px;background:#111;color:#555;"
      "border:1px solid #2a2a2a;font-family:monospace;font-size:11px;"
      "text-align:center;cursor:pointer}"
    ".hop-btn.on{background:#001820;color:#66ccff;border-color:#336688}"
    ".trace-box{background:#000a10;border:1px solid #003344;padding:6px;margin-bottom:6px}"
    ".trace-hop{display:flex;align-items:center;gap:4px;padding:3px 0;"
      "border-bottom:1px solid #001824;font-size:10px}"
    ".trace-hop-name{color:#66ccff;flex:1}"
    ".trace-hop-snr{color:#ffaa00;font-size:9px}"
    ".trace-arrow{color:#335566;font-size:9px}"
    ".trace-pending{color:#ffaa00;font-size:10px}"
    ".trace-timeout{color:#ff4444;font-size:10px}"
    +extra+
    "</style>";
}
...
void handleUiScaleSave() {
  if(server.hasArg("scale")){
    uint8_t s=(uint8_t)server.arg("scale").toInt();
    if(s<=2) uiScale=s;
    saveUiScale();
  }
  server.sendHeader("Location","/settings?subtab=view&saved=1"); server.send(303);
}

// ═══════════════════════════════════════════════════════════════════════════
// [HOP] Hop Limit speichern
// ═══════════════════════════════════════════════════════════════════════════
void handleHopSave() {
  if(server.hasArg("hop")){
    uint8_t h=(uint8_t)server.arg("hop").toInt();
    if(h>=1&&h<=7){ hopLimit=h; saveHopLimit(); }
  }
  server.sendHeader("Location","/settings?subtab=hop&saved=1"); server.send(303);
}
...
void handleTraceStart() {
  ...
  if (nodeId != 0 && nodeId != 0xFFFFFFFF && !traceJob.active)
    sendTracerouteRequest(nodeId);
  server.sendHeader("Location", "/tele?node=" + String(nodeId));
  server.send(303);
}

void handleTraceResult() {
  String html = "<!DOCTYPE html><html><head><meta charset='UTF-8'>";
  html += viewportTag();
  bool stillRunning = traceJob.active;
  if (stillRunning) html += "<meta http-equiv='refresh' content='3;url=/traceresult'>";
  html += frameCss(); html += "</head><body>";

  uint32_t shownTarget = traceJob.active ? traceJob.targetNode : lastTraceResult.targetNode;
  html += "<div style='font-size:9px;padding:3px 0'>"
          "<a href='/tele?node=" + String(shownTarget) + "'>&lt; Node</a> | "
          "<span style='color:#66ccff'>Traceroute</span></div>";

  if (stillRunning) {
    unsigned long elapsed = (millis() - traceJob.sentMs) / 1000;
    html += "<div class='trace-box'>";
    html += "<div class='trace-pending'>Warte auf Antwort von "
          + escHtml(resolveNodeName(traceJob.targetNode))
          + "... (" + String(elapsed) + "s)</div>";
    html += "<div style='color:#335566;font-size:9px;margin-top:4px'>"
            "Kann bis zu " + String(TRACE_TIMEOUT_MS/1000) + "s dauern.</div>";
    html += "</div>";
  } else if (lastTraceResult.valid) {
    html += "<div class='trace-box'>";
    html += "<div style='color:#66ccff;font-size:11px;margin-bottom:6px'>Route zu "
          + escHtml(resolveNodeName(lastTraceResult.targetNode)) + "</div>";

    html += "<div style='color:#888;font-size:9px;margin-bottom:3px'>HINWEG:</div>";
    html += "<div class='trace-hop'><span class='trace-hop-name'>Du (ICH)</span></div>";
    for (int i = 0; i < lastTraceResult.routeCount; i++) {
      html += "<div style='text-align:center' class='trace-arrow'>v</div>";
      html += "<div class='trace-hop'><span class='trace-hop-name'>"
            + escHtml(resolveNodeName(lastTraceResult.route[i])) + "</span>";
      if (i < lastTraceResult.snrTowardsCount)
        html += "<span class='trace-hop-snr'>" + String(lastTraceResult.snrTowards[i]) + " dB</span>";
      html += "</div>";
    }
    html += "<div style='text-align:center' class='trace-arrow'>v</div>";
    html += "<div class='trace-hop'><span class='trace-hop-name' style='color:#00ff66'>"
          + escHtml(resolveNodeName(lastTraceResult.targetNode)) + " (Ziel)</span>";
    if (lastTraceResult.snrTowardsCount > lastTraceResult.routeCount)
      html += "<span class='trace-hop-snr'>"
            + String(lastTraceResult.snrTowards[lastTraceResult.snrTowardsCount-1]) + " dB</span>";
    html += "</div>";

    if (lastTraceResult.routeBackCount > 0 || lastTraceResult.snrBackCount > 0) {
      html += "<div style='color:#888;font-size:9px;margin:8px 0 3px 0'>RUECKWEG:</div>";
      html += "<div class='trace-hop'><span class='trace-hop-name'>"
            + escHtml(resolveNodeName(lastTraceResult.targetNode)) + "</span></div>";
      for (int i = 0; i < lastTraceResult.routeBackCount; i++) {
        html += "<div style='text-align:center' class='trace-arrow'>v</div>";
        html += "<div class='trace-hop'><span class='trace-hop-name'>"
              + escHtml(resolveNodeName(lastTraceResult.routeBack[i])) + "</span>";
        if (i < lastTraceResult.snrBackCount)
          html += "<span class='trace-hop-snr'>" + String(lastTraceResult.snrBack[i]) + " dB</span>";
        html += "</div>";
      }
      html += "<div style='text-align:center' class='trace-arrow'>v</div>";
      html += "<div class='trace-hop'><span class='trace-hop-name' style='color:#00ff66'>Du (ICH)</span>";
      if (lastTraceResult.snrBackCount > lastTraceResult.routeBackCount)
        html += "<span class='trace-hop-snr'>"
              + String(lastTraceResult.snrBack[lastTraceResult.snrBackCount-1]) + " dB</span>";
      html += "</div>";
    } else {
      html += "<div style='color:#444;font-size:9px;margin-top:6px'>"
              "Kein Rueckweg (Firmware &lt; 2.5 oder nicht gemeldet).</div>";
    }
    html += "<div style='color:#334;font-size:8px;margin-top:8px'>"
            + String(lastTraceResult.routeCount) + " Hop(s) hin"
            + (lastTraceResult.routeBackCount>0 ? ", "+String(lastTraceResult.routeBackCount)+" zurueck" : "")
            + "</div>";
    html += "</div>";
    html += "<a class='btn-trace' href='/tracestart?node=" + String(lastTraceResult.targetNode)
          + "'>Erneut traceroute</a>";
  } else {
    html += "<div class='trace-box'>";
    html += "<div class='trace-timeout'>Keine Antwort (Timeout).</div>";
    html += "<div style='color:#335566;font-size:9px;margin-top:4px'>"
            "Node ausser Reichweite oder falscher Kanalschluessel.</div>";
    html += "</div>";
  }
  html += "</body></html>";
...
// ═══════════════════════════════════════════════════════════════════════════
void handleShell(){
  String frameUrl="/chat?tab=0"; activeView="ch1";
  ...
  html+="<html><head><meta charset='UTF-8'>"; html+=viewportTag();
  html+="<title>Mesh PSP v22</title></head>";
...
void handleTabs(){
  ...
  String h="<!DOCTYPE html><html><head><meta charset='UTF-8'>"; h+=viewportTag();
  h+="<style>"
    ...
    ".tab{flex:1;display:flex;align-items:center;justify-content:center;"
      "background:#1a1a1a;color:#556655;text-decoration:none;border:1px solid #2a2a2a;"
      "font-size:10px;font-weight:bold;text-align:center;box-sizing:border-box;"
      "overflow:hidden;text-overflow:ellipsis;letter-spacing:0px;line-height:1}"
  ...
  struct { const char* id; const char* lbl; const char* url; int style; } tabs[]={
    {"ch1","CH1","/?view=ch1",0},{"ch2","CH2","/?view=ch2",0},
    {"pn","PN","/?view=pn",0},{"nodes","Node","/?view=nodes",0},
    {"tele","Tele","/?view=tele",0},{"cam","CAM","/?view=cam",1},
    {"set",secAlert?"SET!":"SET","/?view=set",secAlert?3:2},
  };
  for(auto& t:tabs){
    bool on=(String(t.id)==active); String cls="tab";
...
void handleChat(){
  ...
  String html="<!DOCTYPE html><html><head><meta charset='UTF-8'>"; html+=viewportTag();
  ...
  int start=(messageCount>MAX_MESSAGES)?messageCount%MAX_MESSAGES:0;
  int count=min(messageCount,MAX_MESSAGES); int found=0;
  for(int i=0;i<count;i++){
    int idx=(start+i)%MAX_MESSAGES;
    if(messages[idx].channelIdx!=tab) continue; found++;
    ...
    if(isMe) html+=ackBadge(messages[idx].ackStatus);
    html+="</div></div>";
  }
...
void handleSettings(){
  ...
  html+="<style>"
    ".set-subtab{flex:1;padding:6px 0;background:#111;color:#556655;"
      "text-decoration:none;border:1px solid #2a2a2a;font-size:10px;"
    ...
    ".set-subtab-view.on{background:#050f00;color:#88ff44;border-color:#447700}"
    ".set-subtab-hop.on{background:#000a18;color:#66ccff;border-color:#336688}"
  ...
  auto stab=[&](String id,String lbl,String extra){
    bool on=(subtab==id); String cls="set-subtab";
    if(!extra.isEmpty()) cls+=" "+extra; if(on) cls+=" on";
    html+="<a class='"+cls+"' href='/settings?subtab="+id+"'>"+lbl+"</a>";
  };
  stab("presets","PRESET","");
  stab("hop","HOP","set-subtab-hop");
  stab("wifi","WIFI","set-subtab-wifi");
  stab("sec",secTabLabel,"set-subtab-sec");
  stab("view","VIEW","set-subtab-view");
  html+="</div>";

  if(saved){
    html+="<div style='color:#00ff66;font-size:10px;margin-bottom:8px;"
          "border:1px solid #005500;padding:4px;background:#001200'>[ok] Gespeichert.</div>";
  }

  if(subtab=="hop"){
    html+="<div class='set-section'>";
    html+="<div class='set-label'>HOP LIMIT</div>";
    html+="<div style='color:#334455;font-size:9px;margin-bottom:8px'>"
          "Gilt fuer ALLE Pakete: Nachrichten (CH1/CH2/PN) und Traceroute.<br>"
          "Meshtastic-Standard: 3. Aktuell: "
          "<span style='color:#66ccff;font-weight:bold'>"+String(hopLimit)+"</span>"
          "<br><span style='color:#222;font-size:8px'>"
          "1=nur direkte Nachbarn &nbsp; 3=Standard &nbsp; 7=Maximum</span></div>";

    html+="<form method='POST' action='/hopsave'>";
    html+="<div class='hop-row'>";
    for(int h=1;h<=7;h++){
      bool on=(hopLimit==h);
      html+="<button type='submit' name='hop' value='"+String(h)+"' "
           "class='hop-btn"+String(on?" on":"")+"'>"+String(h)+"</button>";
    }
    html+="</div>";
    html+="<div style='color:#223344;font-size:8px;margin-top:4px'>"
          "Wert anklicken und sofort speichern.</div>";
    html+="</form></div>";
  }
    ...
    html+="<div class='set-section'>";
    html+="<div class='set-label'>WLAN ACCESS POINT</div>";
    html+="<form method='POST' action='/wifisave'>";
    html+="<div style='color:#334455;font-size:9px;margin-bottom:6px'>"
          "SSID: <span style='color:#44aaff'>"+escHtml(wifiSSID)+"</span>"
          " | Versteckt: <span style='color:#44aaff'>"+String(wifiHidden?"JA":"NEIN")+"</span>"
          " | Offen (kein PW)</div>";
    html+="<div class='set-row'><span class='set-label' style='min-width:50px'>SSID</span>";
    html+="<input class='set-inp-wifi' type='text' name='wifi_ssid' value='"+escHtml(wifiSSID)
         +"' maxlength='32' placeholder='Netzwerkname' style='flex:1'></div>";
    html+="<div class='set-row' style='margin-top:4px'>";
    html+="<input type='checkbox' name='wifi_hidden' id='wifi_hidden'"+String(wifiHidden?" checked":"")
         +" style='margin-right:4px'>";
    html+="<label for='wifi_hidden' style='color:#778899;font-size:10px'>Verstecktes WLAN</label></div>";
    html+="<div style='margin-top:8px'><input type='submit' value='Speichern & Neu verbinden' class='set-save-wifi'></div>";
    html+="</form></div>";
  }
  else if(subtab=="view"){
    html+="<div class='set-section'>";
    html+="<div class='set-label'>ANSICHTSGRÖSSE</div>";
    html+="<div style='color:#334433;font-size:9px;margin-bottom:8px'>"
          "Aktuell: <span style='color:#88ff44'>"
          +String(uiScale==0?"PSP":uiScale==1?"Netbook":"Smartphone")+"</span></div>";
    html+="<form method='POST' action='/uiscalesave'><div class='scale-row'>";
    for(int v=0;v<=2;v++){
      const char* lbl[]={"PSP","Netbook","Smartphone"};
      bool on=(uiScale==v);
      html+="<button type='submit' name='scale' value='"+String(v)+"' "
           "class='scale-btn"+String(on?" on":"")+"' "
           "style='border:none;cursor:pointer;font-family:monospace'>"+lbl[v]+"</button>";
    }
    html+="</div></form>";
    html+="<div style='color:#222;font-size:9px;margin-top:6px'>"
          "PSP: 480px nativ | Netbook: 900px | Smartphone: device-width</div>";
    html+="</div>";
  }
  else {
    html+="<div class='set-section'><div class='sec-box'>";
    ...
    if(strlen(secCurrentMac)>0){
      ...
      if(secClientKnown) html+="<div class='sec-ok'>[ok] Bekannt</div>";
      else html+="<div class='sec-blocked'>[!] UNBEKANNT - geblockt</div>";
    } else { html+="<div style='color:#333;font-size:9px'>Kein Client</div>"; }
    html+="</div>";

    html+="<div class='sec-box'><div class='set-label'>HISTORY (letzte "+String(SEC_HISTORY_SIZE)+")</div>";
    bool anyHist=false;
    for(int k=0;k<SEC_HISTORY_SIZE;k++){
      ...
      html+=(secHistory[i].allowed?"<span class='sec-hist-ok'>[ok]</span> ":"<span class='sec-hist-bad'>[!]</span> ");
    ...
    if(!anyHist) html+="<div style='color:#333;font-size:9px'>Keine Verbindungen.</div>";
    html+="</div>";

    html+="<div class='set-label'>MODUS</div>";
    if(!secWhitelistMode){
      html+="<div style='color:#00ccff;font-size:9px;margin-bottom:3px'>[*] Lernmodus</div>";
      html+="<a class='sec-mode-lock' href='/sec_mode?v=1'>-> SPERREN</a>";
    } else {
      html+="<div style='color:#ff6600;font-size:9px;margin-bottom:3px'>[!] Sperrmodus</div>";
      html+="<a class='sec-mode-learn' href='/sec_mode?v=0'>-> LERNEN</a>";
    }
    html+="<hr><div class='set-label'>WHITELIST ("+String(secWhitelistCount)+"/"+String(SEC_MAX_WHITELIST)+")</div>";
    if(secWhitelistCount==0){ html+="<div style='color:#333;font-size:9px'>Leer.</div>"; }
    else {
      for(int i=0;i<secWhitelistCount;i++){
        ...
        html+="<span class='sec-mac' style='flex:1;color:"+String(isCur?"#00ff66":"#ffcc88")+"'>"
             +escHtml(secWhitelist[i].mac);
        if(isCur) html+=" <span style='color:#00ff66;font-size:8px'>[DU]</span>";
        html+="</span><a class='sec-btn-red' href='/sec_wl_rm?i="+String(i)+"'>[x]</a></div>";
      }
    }
    html+="</div>";
  }
  ...
  html+="<a href='/shutdown' style='display:block;background:#1a0000;color:#ff4444;"
        ...
        "text-align:center;text-decoration:none;margin-top:4px'>[off] ESP32 herunterfahren</a></div>";
  html+="<hr><div style='color:#333;font-size:9px;margin-top:4px'>Mesh PSP v22</div>";
  html+="</body></html>";
  server.send(200,"text/html; charset=utf-8",html);
}
...
// ═══════════════════════════════════════════════════════════════════════════
// Send / Clear / PN / Nodes / Telemetry / Camera (unveränderter Kern)
// ═══════════════════════════════════════════════════════════════════════════
void handleSend(){
  uint8_t tab=0;
  if(server.hasArg("tab")) tab=(uint8_t)server.arg("tab").toInt();
...
void handlePNList(){
  ...
  if(pnCount==0){ html+="<div class='empty'>Keine Privatnachrichten.</div>"; }
  else {
    for(int i=0;i<pnCount;i++){
      uint32_t partner=pnPartners[i]; const char* name=resolveNodeName(partner);
      const char* preview="";
...
void handlePNConv(){
  ...
  for(int i=0;i<count;i++){
    ...
    if(partner!=nodeId) continue; found++;
    bool isMe=(messages[idx].senderId==0);
    bool isSys=(strcmp(messages[idx].senderName,"SYS")==0);
    if(strncmp(messages[idx].text,"FOTO:",5)==0){
      ...
      if(!isMe){
        ...
        if(s&&s->complete)
          ...
          if(missing>0){ html+=" <span style='color:#ffaa00;font-size:9px'>("+String(missing)+" fehlend";
    ...
    if(isSys){
      ...
      html+="<div class='txt' style='color:#555'>"+escHtml(messages[idx].text)+"</div></div>"; continue;
    }
...
void handleNodeList(){
  ...
  html+=String(nodeCacheCount)+" Nodes &nbsp;<a href='/refreshnodes' target='content'>[Refresh]</a></div>";
...
void handleTelemetry(){
  ...
  bool ra2=traceJob.active&&traceJob.targetNode==nodeId?true:ra;
  unsigned long refreshSecs=(traceJob.active&&traceJob.targetNode==nodeId)?3:10;
  String self=base+"refresh="+String(ra?"1":"0");
  String html="<!DOCTYPE html><html><head><meta charset='UTF-8'>"; html+=viewportTag();
  if(ra2) html+="<meta http-equiv='refresh' content='"+String(refreshSecs)+";url="+self+"'>";
  html+=frameCss(); html+="</head><body>";
  html+=refreshBar(ra,base);
  ...
  html+="<div class='trace-box'><div class='set-label'>TRACEROUTE (HopLimit: "
       +String(hopLimit)+")</div>";
  if(traceJob.active&&traceJob.targetNode==nodeId){
    unsigned long elapsed=(millis()-traceJob.sentMs)/1000;
    html+="<div class='trace-pending'>Anfrage laeuft... ("+String(elapsed)+"s)</div>";
    html+="<a class='btn-trace' href='/traceresult' target='content' style='margin-top:4px;display:inline-block'>Ergebnis ansehen</a>";
  } else if(lastTraceResult.valid&&lastTraceResult.targetNode==nodeId){
    html+="<div style='color:#66ccff;font-size:9px;margin-bottom:4px'>"
          +String(lastTraceResult.routeCount)+" Hop(s) - Ergebnis verfuegbar</div>";
    html+="<a class='btn-trace' href='/traceresult' target='content'>Route ansehen</a> ";
    html+="<a class='btn-trace' href='/tracestart?node="+String(nodeId)+"'>Erneut starten</a>";
  } else {
    html+="<a class='btn-trace' href='/tracestart?node="+String(nodeId)+"'>Traceroute starten</a>";
  }
  html+="</div>";
...
void handleClear(){
  ...
  for(int i=0;i<MAX_MESSAGES;i++){
    if(isPN){ uint32_t p=(messages[i].senderId==0)?messages[i].targetId:messages[i].senderId;
      if(messages[i].channelIdx==2&&p==partnerId){messages[i].channelIdx=99;messages[i].text[0]='\0';}
    } else { if(messages[i].channelIdx==tab){messages[i].channelIdx=99;messages[i].text[0]='\0';} }
  }
...
void handleCamera(){
  ...
  if(photoJob.active) html+=" &nbsp;<span class='chunk-bar'>Sendet "+String(photoJob.nextChunk)+"/"+String(photoJob.totalChunks)+"</span>";
  html+="</div><div class='qual-row'>";
  for(uint8_t q=0;q<=2;q++){
    String href="/cam?qual="+String(q)+(activePnPartner?"&pnnode="+String(activePnPartner):"");
    html+="<a class='qual-btn"+String(q==camQuality?" on":"")+"' href='"+href+"'>"+String(qNames[q])
  ...
  if(capturedJpeg!=nullptr&&capturedJpegLen>0){
    ...
    html+="<div class='cam-status'>"+String(capturedW)+"x"+String(capturedH)+" | "+String(capturedJpegLen)
         +" Bytes | "+String(chunks)+" Chunks | ~"+String((unsigned long)chunks*5)+"s</div>";
    ...
    if(!photoJob.active&&!retryJob.active){
      if(pnCount==0) html+="<div style='color:#444;font-size:9px;margin-top:4px'>Keine offenen PN.</div>";
      else if(pnCount==1&&activePnPartner==pnPartners[0])
      ...
      String ji=photoJob.active?"Sende "+String(photoJob.nextChunk)+"/"+String(photoJob.totalChunks)
                               :"Retry "+String(retryJob.nextIdx)+"/"+String(retryJob.missingCount);
      html+="<div style='color:#ffaa00;font-size:10px;margin-top:4px'>"+ji+"</div>";
    }
    html+="</div>";
  } else { html+="<div class='empty'>Noch kein Foto.</div>"; }
  bool anyRcv=false;
  ...
  if(anyRcv){
    ...
    for(int i=0;i<MAX_RECV_SESSIONS;i++){
      ...
      if(s->complete)
        html+="<a class='foto-link' href='/photoview?session="+String(sx)+"' target='content'>"
             "Ansehen ("+String(s->imgW)+"x"+String(s->imgH)+")</a>";
      else
        html+="<span style='color:#888'>"+String(s->receivedCount)+"/"+String(s->totalChunks)+" Chunks</span>";
      html+="</div>";
    }
  }
  html+="</body></html>"; server.send(200,"text/html; charset=utf-8",html);
...
void handleCamSnap(){
  if(server.hasArg("pnnode")) activePnPartner=strtoul(server.arg("pnnode").c_str(),NULL,10);
  String snapStatus="";
  if(!cameraReady){ snapStatus="<div style='color:#ff4444'>Kamera nicht bereit.</div>"; }
  else {
    if(capturedJpeg!=nullptr){free(capturedJpeg);capturedJpeg=nullptr;capturedJpegLen=0;}
    cameraReady=initCamera(camQuality);
    if(!cameraReady){ snapStatus="<div style='color:#ff4444'>Re-Init fehlgeschlagen.</div>"; }
    else {
      sensor_t* s=esp_camera_sensor_get();
  ...
  html+=frameCss(); html+="</head><body>";
  html+=snapStatus;
  html+="<div style='color:#555;font-size:9px;margin-top:4px'>Zurueck...</div>";
  html+="</body></html>"; server.send(200,"text/html; charset=utf-8",html);
}
...
void handleCamPreview(){
  ...
  html+="<div style='font-size:9px;color:#555;padding:3px 0'>"
       +String(capturedW)+"x"+String(capturedH)+" &nbsp;<a href='/cam'>zurueck</a></div>";
