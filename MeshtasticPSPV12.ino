// ╔══════════════════════════════════════════════════════════════════════════╗
// ║  Meshtastic PSP  —  v12 (final)                                          ║
// ║  Erstellt:  2026-06-12                                                   ║
// ║  Timestamp: 2026-06-12 00:41 UTC  (build: 2026-06-12T00:41:29Z)         ║
// ║                                                                          ║
// ║  Änderungen gegenüber v11:                                               ║
// ║  - Neuer Settings-Tab (SET) für Preset-Button-Konfiguration              ║
// ║  - 9 Mini-Buttons auf CH2 in einer Zeile (flex, 5-Zeichen-Label)         ║
// ║  - Leere Slots zeigen b1..b9, Klick führt zu Settings                    ║
// ║  - Preset-Texte in NVS (Preferences), handleSettings/handleSettingsSave  ║
// ║                                                                          ║
// ║  Änderungen v12 final:                                                   ║
// ║  - PHOTO_SEND_DELAY_MS 2000 → 5000 (5s zwischen Chunks)                  ║
// ║  - 320px (quality 0+1): Graustufeneffekt erzwungen (weniger Chunks)      ║
// ║  - Settings-Inhalt NUR im SET-Tab sichtbar (kein externer Link nötig)    ║
// ║  - Timestamp im Header für exakte Versionseinordnung                     ║
// ║                                                                          ║
...
#define PHOTO_SEND_DELAY_MS 5000   // v12final: 5s zwischen Chunks (war 2000)

// ═══════════════════════════════════════════════════════════════════════════
// PRESET BUTTONS  (9 Stück, CH2)
// ═══════════════════════════════════════════════════════════════════════════
...
// Qualität: 0=Hoch(320x240 Q12 SW)  1=Mittel(320x240 Q20 SW)  2=Preview(160x120 Q25 Farbe)
// v12final: quality 0+1 => Graustufen erzwungen (kleinere JPEG, weniger Chunks)
...
void setup() {
  ...
  delay(1500);
  Serial.println("--- Meshtastic PSP v12final (2026-06-12T00:41:29Z) ---");
...
// ═══════════════════════════════════════════════════════════════════════════
// KAMERA INIT
// v12final: quality 0+1 (320x240) => Graustufen (2=Grayscale)
//           quality 2   (160x120) => Normal/Farbe
// ═══════════════════════════════════════════════════════════════════════════
bool initCamera(uint8_t quality) {
  esp_camera_deinit();
  delay(150);
  ...
  switch (quality) {
    case 1:  fs=FRAMESIZE_QVGA;  jpegQ=20; break;
    case 2:  fs=FRAMESIZE_QQVGA; jpegQ=25; break;
  ...
  if (s) {
    ...
    s->set_sharpness(s, 1);
    s->set_denoise(s, 1);
    // v12final: 320px (quality 0+1) => Graustufen -> kleinere JPEG, weniger Chunks
    //           160px (quality 2)   => Normal/Farbe
    if(quality <= 1) s->set_special_effect(s, 2); // 2 = Grayscale
    else             s->set_special_effect(s, 0); // 0 = Normal/Farbe
  }
...
void loadPresets() {
  const char* defaults[PRESET_COUNT] = {
    "go","stp","wg2","arm","drm","","","",""
  };
  for(int i=0;i<PRESET_COUNT;i++){
    char key[12]; snprintf(key,sizeof(key),"prs%d",i);
  ...
  Serial.println("[Presets] Geladen.");
}
...
// ═══════════════════════════════════════════════════════════════════════════
// FOTO SENDE-TICKER  — v12final: 5000ms Delay zwischen Chunks
// ═══════════════════════════════════════════════════════════════════════════
void tickPhotoSend(){
...
void handleShell(){
  ...
  html+="<html><head><meta charset='UTF-8'><title>Mesh PSP v12</title></head>";
...
void handleTabs(){
  ...
  h+="<style>"
    "body{background:#111;margin:0;padding:2px 1px;font-family:monospace;"
      "white-space:nowrap;overflow:hidden;display:flex;gap:1px}"
    ".tab{flex:1;padding:10px 0;background:#1a1a1a;color:#556655;"
...
void handleChat(){
  ...
  if(tab==1){
    ...
    for(int i=0;i<PRESET_COUNT;i++){
      if(strlen(presetTexts[i])>0){
        char lbl[7]; strncpy(lbl,presetTexts[i],5); lbl[5]='\0';
        html+="<a class='preset-btn' href='/send?target=BROADCAST_CH2&tab=1&msg="+
              String(presetTexts[i])+"&refresh=1' target='_self'>"+
              escHtml(lbl)+"</a>";
      } else {
        char lbl[5]; snprintf(lbl,sizeof(lbl),"b%d",i+1);
        html+="<a class='preset-btn-empty' href='/settings' target='content'>"+String(lbl)+"</a>";
      }
    }
    html+="</div>";
  }
...
// ═══════════════════════════════════════════════════════════════════════════
// SETTINGS — nur über SET-Tab erreichbar
// v12final: kein direkter externer Link; Inhalt nur im content-Frame sichtbar
// ═══════════════════════════════════════════════════════════════════════════
void handleSettings(){
  ...
  html+=frameCss();
  html+="</head><body>";
  html+="<div style='color:#cccc00;font-size:11px;font-weight:bold;margin-bottom:6px'>"
        "SETTINGS &mdash; <span style='color:#445544;font-size:9px'>"
        "v12final 2026-06-12T00:41:29Z</span></div>";
  ...
  html+="<div class='set-label'>QUICK-SEND BUTTONS &mdash; CH2 (9 Preset-Slots)</div>";
  ...
  for(int i=0;i<PRESET_COUNT;i++){
    ...
    html+="<input class='set-inp' type='text' name='p"+String(i)+"' "
          ...
          "placeholder='leer = Button ausgeblendet'>";
    html+="</div>";
  }
  html+="<div style='margin-top:8px'>";
  html+="<input type='submit' value='Alle speichern' class='set-save'>";
  html+="</div>";
  html+="</form>";
  html+="</div>";

  html+="<hr>";
  html+="<div style='color:#333;font-size:9px;margin-top:4px'>"
        "Mesh PSP v12final &mdash; 2026-06-12T00:41:29Z</div>";
  html+="</body></html>";
  server.send(200,"text/html; charset=utf-8",html);
}
...
// ═══════════════════════════════════════════════════════════════════════════
// KAMERA HAUPTSEITE
// v12final: Qualitätsbezeichnungen zeigen SW-Hinweis bei 320px
// ═══════════════════════════════════════════════════════════════════════════
void handleCamera(){
  ...
  // v12final: 320px-Modi als SW gekennzeichnet
  const char* qNames[3]={"Hoch SW (320x240)","Mittel SW (320x240)","Preview Farbe (160x120)"};
  const char* qInfo[3] ={"Q12 ~2-4KB","Q20 ~1-3KB","Q25 ~1-2KB"};
  ...
  if(capturedJpeg!=nullptr&&capturedJpegLen>0){
    ...
    if(!photoJob.active){
      ...
      html+="<div style='color:#ffaa00;font-size:10px;margin-top:4px'>Übertragung läuft... "
           +String(photoJob.nextChunk)+"/"+String(photoJob.totalChunks)
           +" &nbsp; noch ~"+String((photoJob.totalChunks-photoJob.nextChunk)*5)+"s</div>";
    }
    html+="</div>";
  } else {
