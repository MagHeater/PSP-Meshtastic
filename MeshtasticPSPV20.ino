...
// ║  Meshtastic PSP  —  v21                                                  ║
// ║  Erstellt:  2026-06-27                                                   ║
// ║  Änderungen gegenüber v20:                                               ║
// ║                                                                          ║
// ║  [PKTID-FIX] KRITISCHER BUG in v20 gefunden: pkt.id wurde mit einem      ║
// ║   simplen Zähler ab 1,2,3,... vergeben. Die Meshtastic-Firmware führt   ║
// ║   intern eine PacketHistory (Dedup-Tabelle) über (from,id)-Paare. Mit   ║
// ║   solchen kleinen IDs kollidiert der Client mit IDs, die das Radio      ║
// ║   selbst bereits intern erzeugt/gesehen hat -> Firmware hält das Paket  ║
// ║   für ein Duplikat und verwirft es VOR dem Senden. Resultat in v20:     ║
// ║   keine Nachrichten kommen mehr raus (nicht mal ohne ACK).             ║
// ║                                                                          ║
// ║  [PKTID-FIX] Lösung: pkt.id wird wie bei Meshtastic selbst aus einem    ║
// ║   zufälligen Seed (esp_random()) + Inkrement gebildet, NICHT aus einer  ║
// ║   kleinen Zählerreihe ab 1. Dadurch keine Kollision mit interner       ║
// ║   PacketHistory, ACK-Matching (pkt.id == ACK.request_id) bleibt erhalten║
// ║                                                                          ║
// ║  [ACK] (aus v20 übernommen, weiterhin gültig):                          ║
// ║   - pkt.id = reqId beim Senden (Meshtastic ACK = ROUTING_APP Paket mit  ║
// ║     decoded.request_id == pkt.id der ursprünglichen Nachricht)         ║
// ║   - data.request_id beim Senden NICHT gesetzt (gehört da nicht hin)    ║
// ║                                                                          ║
// ║  [DISCO] sendDisconnect() vor Deep Sleep                                 ║
// ║  [DISCO] XIAO nRF bekommt sauberes Disconnect-Signal                     ║
// ║  [DISCO] 300ms Wartezeit damit XIAO Session zurücksetzt                  ║
// ║                                                                          ║
// ║  OV3660-Kamera-Fix: Belichtungseinstellungen von PSP WebCam v2.0         ║
// ║   IF WIFI DOESNT INITIALIZE, FLASH V12, SAVE BUTTONS, FLASH NEWEST VER   ║
// ║                                                                          ║
...
// ACK TRACKING
// ═══════════════════════════════════════════════════════════════════════════
// Jede gesendete Nachricht bekommt eine ID die als pkt.id gesetzt wird.
// Meshtastic sendet ACK zurück als ROUTING_APP Paket mit
// decoded.request_id == ursprüngliche pkt.id.
// [PKTID-FIX] Diese ID MUSS aus einem zufälligen Wertebereich kommen,
// sonst kollidiert sie mit der internen PacketHistory der Firmware
// und das Paket wird beim Senden bereits als Duplikat verworfen.
...
struct ChatMessage {
  ...
  uint8_t  channelIdx;
  // [ACK] Felder
  uint32_t requestId;   // == pkt.id der gesendeten Nachricht (0 = kein ACK erwartet)
  bool     acked;       // true wenn ACK empfangen
};
...
// ═══════════════════════════════════════════════════════════════════════════
// [PKTID-FIX] Zähler für pkt.id — wird in setup() mit esp_random() geseedet,
// damit der Wertebereich NICHT mit der internen PacketHistory der
// Meshtastic-Firmware kollidiert (kein 1,2,3,... mehr, kein XOR-Wrap mehr).
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// [PKTID-FIX] Erzeugt eine Paket-ID im Stil von Meshtastics eigener
// generatePacketId(): ein zufällig geseedeter, monoton steigender Zähler.
// Dadurch keine Kollision mit IDs, die die Firmware selbst intern bereits
// in ihrer PacketHistory (Dedup-Tabelle) verwaltet.
// ═══════════════════════════════════════════════════════════════════════════
uint32_t nextPacketId(){
  uint32_t id = gPacketIdCounter++;
  if (id == 0) id = gPacketIdCounter++; // 0 ist reserviert ("kein ACK erwartet")
  return id;
}
...
void setup() {
  ...
  delay(1500);
  Serial.println("--- Meshtastic PSP v21 ---");

  // [PKTID-FIX] Zähler mit Zufallswert seeden — NICHT bei 1 starten.
  // Verhindert Kollision mit der internen PacketHistory/Dedup der
  // Meshtastic-Firmware, die sonst eigene Pakete als Duplikate verwirft.
  randomSeed(esp_random());
  gPacketIdCounter = (esp_random() % 0x3FFFFFFFUL) + 0x10000UL; // großer, zufälliger Startwert
...
void sendDisconnect(){
  ...
  if(pb_encode(&s,meshtastic_ToRadio_fields,&tr)){
    ...
    Serial2.write((s.bytes_written>>8)&0xFF); Serial2.write(s.bytes_written&0xFF);
    Serial2.write(buf,s.bytes_written); Serial2.flush();
    Serial.println("[DISCO] Disconnect gesendet an XIAO nRF");
  }
}
...
// ═══════════════════════════════════════════════════════════════════════════
// [ACK] markAcked — sucht Nachrichten mit passender requestId und setzt acked=true
// ═══════════════════════════════════════════════════════════════════════════
void markAcked(uint32_t requestId) {
  ...
  for (int i = 0; i < total; i++) {
    if (messages[i].requestId == requestId && !messages[i].acked) {
      ...
      Serial.printf("[ACK] Nachricht requestId=0x%08X als zugestellt markiert\n", requestId);
    }
  }
}
...
void processFromRadio(uint8_t* buf,uint16_t len){
  ...
  if(fr.which_payload_variant==meshtastic_FromRadio_packet_tag){
    ...
    // ── ACK-Erkennung ──────────────────────────────────────────────────────
    // Meshtastic sendet ACKs als decoded MeshPacket mit portnum=ROUTING_APP.
    // decoded.request_id == pkt.id der ursprünglich gesendeten Nachricht
    // (= der Wert den wir als requestId gespeichert haben).
    if(pkt.which_payload_variant==meshtastic_MeshPacket_decoded_tag){
      meshtastic_Data data=pkt.decoded;

      // ── ROUTING_APP = ACK oder Error-Response ────────────────────────────
      if(data.portnum==meshtastic_PortNum_ROUTING_APP){
        if(data.request_id != 0){
          Serial.printf("[ACK] ROUTING_APP empfangen von 0x%08X, request_id=0x%08X\n",
                        pkt.from, data.request_id);
          markAcked(data.request_id);
        }
        // Routing-Pakete nicht als Chat-Nachricht behandeln
        return;
      }
      ...
      // encoded Paket (verschlüsselt oder nicht dekodiert)
      NodeCache* sn=findNode(pkt.from);
      if(!sn){cacheNode(pkt.from,"","");sn=findNode(pkt.from);}
      if(sn){sn->snr=pkt.rx_snr;sn->hasSnr=true;sn->lastSeen=millis();}
    }
...
// SENDEN
// ═══════════════════════════════════════════════════════════════════════════
// [ACK-FIX] pkt.id = reqId  ← das ist der Wert den Meshtastic im ACK als
//   request_id zurückschickt. data.request_id wird NICHT gesetzt.
// [PKTID-FIX] reqId kommt aus nextPacketId() = zufällig geseedeter Zähler,
//   damit es NICHT mit der internen PacketHistory/Dedup der Firmware
//   kollidiert (das war der Grund warum in v20 GAR KEINE Nachricht mehr
//   raus ging: kleine IDs 1,2,3,... wurden von der Firmware als bereits
//   gesehenes Duplikat verworfen, noch bevor sie über Funk gingen).
// ═══════════════════════════════════════════════════════════════════════════
void sendToRadio(const char* text,uint32_t targetDst,uint8_t channelIdx){
  meshtastic_ToRadio    tr=meshtastic_ToRadio_init_zero;
  ...
  // Nur für echte Textnachrichten (keine Foto-Chunks) ACK anfordern
  bool wantAck = (strncmp(text,"IMG:",4)!=0 &&
                  strncmp(text,"IMGDONE:",8)!=0 &&
                  strncmp(text,"IMGREQ:",7)!=0);

  // [PKTID-FIX] pkt.id aus zufällig geseedetem Zähler, nicht aus 1,2,3,...
  uint32_t reqId = 0;
  if (wantAck) {
    reqId = nextPacketId();
  }
  ...
  // [ACK-FIX] pkt.id setzen (NICHT data.request_id!)
  ...
  if(wantAck){
    ...
    // requestId = pkt.id (der Wert den Meshtastic im ACK zurückschickt)
    messages[idx].requestId = reqId;
    messages[idx].acked     = false;
...
String frameCss(){
  return "<style>"
    ...
    ".ack-ok{color:#00ff66;font-size:9px;margin-left:4px;font-weight:bold}"
    ".ack-pend{color:#555555;font-size:9px;margin-left:4px}"
...
void handleShell(){
  ...
  html+="<html><head><meta charset='UTF-8'><title>Mesh PSP v21</title></head>";
...
void handleChat(){
  ...
  for(int i=0;i<count;i++){
    ...
    if(isMe && messages[idx].requestId != 0){
      if(messages[idx].acked)
        html+="<span class='ack-ok'>(OK)</span>";
      else
        html+="<span class='ack-pend'>(...)</span>";
    }
    html+="</div></div>";
  }
...
void handleSettings(){
  ...
  html+="<div style='color:#2a1010;font-size:8px;margin-top:3px'>"
        "Disconnect &rarr; Deep Sleep &mdash; Neustart nur per Reset/Strom</div>";
  html+="</div>";
